...
class Player(BasePlayer):
    ...
    bid_amount = models.CurrencyField(doc='Amount bidded by the player', label='Bid amount', min=C.EVALUATE_MIN)
    is_winner = models.BooleanField(doc='Indicates whether the player is the winner', initial=False)
def set_payoff(player: Player):
    group = player.group
    if player.is_winner:
        player.payoff = group.item_value - group.second_highest_bid
        #if player.payoff < 0:
          #  player.payoff = 0
    else:
        player.payoff = 0
